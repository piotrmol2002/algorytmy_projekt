...
def plot_profit_breakdown(results: Dict[str, Any]) -> str:
    ...
    Pokazuje: przychód, koszty, zysk netto - przed i po optymalizacji.
    
    Args:
        results: Pełne wyniki optymalizacji z kosztem typu 'profit_breakdown'
    
    Returns:
        Base64 encoded string z obrazem (lub pusty jeśli nie ma danych)
    """
...
def plot_weighted_radar(baseline: Dict[str, Any], optimized: Dict[str, Any]) -> str:
    """
    Wykres radarowy (pająk) dla funkcji weighted_objective.
    Porównuje znormalizowane metryki przed i po optymalizacji.
    ...
    categories = ['Czas\nodpowiedzi', 'Przepustowość', 'Długość\nkolejki', 'Wykorzystanie']
    
    # Normalizuj metryki (im wyższy tym lepiej, 0-1)
    ...
    # Długość kolejki - mniejsza = lepsza
    ...
    # Zamknij wykres (dodaj pierwszy element na końcu)
    ...
    ax.set_ylim(0, 1)
    ax.set_title('Porównanie metryk (Wykres radarowy)\nFunkcja: weighted_objective', 
...
def generate_all_plots(results: Dict[str, Any]) -> Dict[str, str]:
    ...
    # NOWE: Wykres breakdown zysku (dla profit)
    profit_img = plot_profit_breakdown(results)
    if profit_img:
        plots['profit_breakdown'] = profit_img
    
    # NOWE: Wykres radarowy (dla weighted_objective)
